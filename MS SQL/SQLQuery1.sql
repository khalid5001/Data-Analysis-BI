...
/*
				  'JOIN'     
										*/
--- 1 Retrieve the item_name, order_date, and order_time for all items in the order_details table, including their respective menu item details.
...
SELECT category, AVG(price) AS event FROM menu_items 
GROUP BY category
HAVING AVG(price) > 15;
--- 2 Include the count of items in each category.
SELECT category, AVG(price)AS AVG_Price, COUNT(*) AS event FROM menu_items 
GROUP BY category
HAVING AVG(price) > 15;
